...
time = np.array([])
t0 = 0
dt = 0.2  # timestep analysed, to be tied into back projection time stamp
...
for topic, msg, t in bag.read_messages(topics=['/imu/imu']):
    ...
    if current_time > 0.2:
        # calculate metric
        roughness_metric = (np.average(running_squared_data, returned=True))[0] * 10.0
...
ax.set_title('IMU-based roughness metric')
